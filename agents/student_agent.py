...
import time
from helpers import get_directions, count_capture_dir, execute_move, get_valid_moves
...
class StudentAgent(Agent):
    ...
    def heuristics(self, board, player, opponent):
        ...
        for r in range(M):
            for c in range(M):
                if board[r, c] == player:
                    unflip_h += 5 if self.is_unflippable(board, r, c, M) else 0
    ...
    def is_unflippable(self, board, r, c, M):
